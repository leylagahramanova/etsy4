...
const Main = () => {
  ...
  const [favCartItems, setFavCartItems] = useState([]);
  ...
  useEffect(() => {
    if (products.length > 0 && cart.length > 0) {
      const categories = [...new Set(cart.map(item => item.category?.slug).filter(Boolean))];
      const subcategories = [...new Set(cart.map(item => item.subcategory?.slug).filter(Boolean))];
      // Recommend products from the same categories or subcategories, not in cart
      ...
      setSimilarItems(similar.slice(0, 10)); // Limit to 10
    } else {
      setSimilarItems([]);
    }
  }, [products, cart]);

  // Fetch unique items from Favorites and Cart
  useEffect(() => {
              ...
              >
                <div className="relative group ">
                  <img
                    ...
                    alt={item.name}
                    className="w-[60rem] h-[15rem] object-cover p-1"
                  />
                     <span
            ...
            {similarItems.map((item) => (
              ...
              >
                <div className="relative">
                  <img
                    className="object-cover w-[60rem] h-[15rem]  p-1"
        ...
        <ul className="list-none grid grid-cols-2 md:grid-cols-4   gap-2 ">
          {products.slice(0, 8).map((item, index) => (
            <li key={item.id || index}>
              <div>
                <div
                  className="relative border bg-transparent mr-0 object-cover rounded-2xl cursor-pointer group"
                  onClick={() => navigate(`/product/${item.id}`)}
                >
                  <img
                    className="object-cover w-[60rem] h-[15rem] p-1"
                  ...
                  {item.subcategory?.slug && (
                    <button
                      className="absolute bottom-2 left-1/2 transform -translate-x-1/2 bg-white rounded-full p-2 shadow-md flex items-center 
             transition-all duration-300 group-hover:pr-6"
                    >
                      <span>{item.subcategory.slug}</span>
                      <span className="inline-block text-gray-600 opacity-0 group-hover:opacity-100 transform group-hover:translate-x-1 transition duration-300 ml-1">
            ...
            {products.slice(5, 8).map((item, index) => (
              ...
              >
                ...
                <div onClick={() => navigate(`/product/${item.id}`)} className=' relative'>
                  <img
                    className="object-cover rounded-3xl w-[60rem] lg:h-[20rem] sm:h-[15rem] "
                    src={item.images?.[0] || "https://via.placeholder.com/200"}
                    alt={item.name}
                    style={{ aspectRatio: "0.8" }}
                  />
              ...
              >
                <div className="py-8">
                  <div className="relative inline-block" onClick={() => navigate(`/product/${item.id}`)}>
                    <img
                      className="object-cover rounded-3xl w-[200px] sm:w-45"
                      src={item.images?.[0] || "https://via.placeholder.com/200"}
                      alt={item.name}
                    />
          ...
          {products.slice(1, 6).map((product, index) => (
            ...
            >
              <img
                ...
                className="object-cover  w-[60rem]  h-[15rem] "
              />
              <span
                className="absolute top-2 right-2 hidden group-hover:flex fade-slide-up items-center justify-center w-8 h-8 rounded-full bg-white cursor-pointer"
              >
            ...
            >
              ...
              <div className=" flex flex-col items-center" onClick={() => navigate(`/product/${item.id}`)}>
                <img
                  ...
                  className="object-cover w-[60rem]  h-[15rem]   rounded-2xl mx-auto"
                />
